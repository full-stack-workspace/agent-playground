...
import { getHooks, subagentConfig } from "./agent.config";
...
import { inputArticles } from "./input-data";
...
export async function chat(): Promise<void> {
    ...
    console.log(chalk.cyan(`\nRegistered Subagents: ${Object.keys(subagentConfig).join(", ")}`));
    ...
    try {
        while (true) {
            ...
            const result: Query = query({
                prompt: `${userInput}, 输入的文章数据是：${JSON.stringify(inputArticles)}`,
                options: {
                    // 会话 id
                    resume: sessionId,
                    // lead agent 系统提示词
